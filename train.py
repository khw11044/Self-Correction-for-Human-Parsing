...
def get_arguments():
    ...
    # Data Preference
    parser.add_argument("--data-dir", type=str,default='./input_img2')  # 인풋 데이터
    parser.add_argument("--anno-dir", type=str,default='./input_seg2')  # 인풋 데이터
    ...
    # default='./log/checkpoint_150.pth.tar'
    parser.add_argument("--model-restore", type=str,
                        default='./checkpoints/checkpoint_150.pth.tar')
    parser.add_argument("--schp-start", type=int,
                        default=50, help='schp start epoch')
    parser.add_argument("--cycle-epochs", type=int,
                        default=10, help='schp cyclical epoch')
    # default='./log/checkpoint_150.pth.tar'
    parser.add_argument("--schp-restore", type=str,
                        default='./checkpoints/schp_6_checkpoint.pth.tar')
    parser.add_argument("--lambda-s", type=float, default=1,
                        help='segmentation loss weight')
    parser.add_argument("--lambda-e", type=float,
                        default=1, help='edge loss weight')
...
def main():
    ...
    AugmentCE2P = networks.init_model(
        args.arch, num_classes=args.num_classes, pretrained=args.imagenet_pretrain)
    ...
    SCHP_AugmentCE2P = networks.init_model(
        args.arch, num_classes=args.num_classes, pretrained=args.imagenet_pretrain)
    ...
    train_dataset = LIPDataSet(
        args.data_dir, args.anno_dir, 'train', crop_size=input_size, transform=transform)
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for i_iter, batch in enumerate(train_loader):
            ...
            # images.shape: torch.Size([2, 3, 473, 473])
            ...
            # len(preds): 2, labels.shape : [2,473,473], edges.shape : [2,473,473], soft_preds: None, soft_edges: None, cycle_n : 0
            loss = criterion(
                preds, [labels, edges, soft_preds, soft_edges], cycle_n)       # 여기 오류
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()

            if i_iter % 10 == 0:
                iter10_time = round((time.time() - start_iter_time) / 6, 3)
                if write_total_time == False:
                    toal_time = iter10_time * (total_iters/10)
                    write_total_time = True
                timer += iter10_time
                print('iter = {} of {} ({}%) completed, ({} m/ {} m) ,lr = {}, loss = {}'.format(i_iter, total_iters, round((i_iter / total_iters)*100,3) , round(timer,3), toal_time,
                                                                                        lr, loss.data.cpu().numpy()))
        ...
        print('epoch = {} of {} completed using {} m'.format(epoch, args.epochs, round(((end - start) / (epoch - start_epoch + 1)) / 60 ),2))
