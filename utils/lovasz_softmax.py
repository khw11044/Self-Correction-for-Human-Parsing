...
def flatten_probas(probas, labels, ignore=None):
    ...
    """
    if probas.dim() == 3:               # ([2, 16, 473, 473])
        # assumes output of a sigmoid layer
        B, H, W = probas.size()
        probas = probas.view(B, 1, H, W)
    B, C, H, W = probas.size()           # torch.Size([2, 15, 473, 473])               # 여기서 오류 
    probas = probas.permute(0, 2, 3, 1).contiguous().view(-1, C)  # B * H * W, C = P, C
    labels = labels.view(-1)        # labels.shape : 447458
    if ignore is None:
        return probas, labels
    valid = (labels != ignore)          # tensor([False, False, False,  ..., False, False, False], device='cuda:0')
    vprobas = probas[valid.nonzero().squeeze()]         # vprobas.shape : ([329520, 16])
    vlabels = labels[valid]                             # len(vlabels) : 329520
...
class LovaszSoftmax(nn.Module):
    ...
    def forward(self, pred, label):         # pred.shape : [2, 15, 473, 473]
        pred = F.softmax(pred, dim=1)
        return self.lovasz_softmax(pred, label, per_image=self.per_image, ignore=self.ignore_index, weighted=self.weighted) # pred : ([2, 16, 473, 473])